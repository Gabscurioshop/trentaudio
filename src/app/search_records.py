...
def display_records(r_type, order):
    conn = None
    rows = ''
    try:
        ...
        if r_type == 'Transcript Error':
            cur.execute("CREATE VIEW T_ERRORS AS SELECT REPORT_NUM, AUDIO_ID, USR_EMAIL, ERROR_DESCRIPTION, DATE_CREATED, DECISION FROM (REPORT NATURAL JOIN TRANSCRIPT_ERROR) WHERE (DECISION <> 'Disapproved' AND DECISION <> 'Approved')")
            ...
            cur.execute("CREATE VIEW MD_EDITS AS SELECT REPORT_NUM, AUDIO_ID, USR_EMAIL, EDIT_METADATA_DESCRIPTION, DATE_CREATED, DECISION FROM (REPORT NATURAL JOIN METADATA_EDIT_REQUEST) WHERE (DECISION <> 'Disapproved' AND DECISION <> 'Approved')")
            if order == 'Recent First':
